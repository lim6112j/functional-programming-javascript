...
const length = ([h, ...tail], len = 0) =>
  def(h) ? length(tail, len + 1) : len; // tail-recursive
...
const first = ([h, ...xs], n = 1, acc = []) =>
  n < 1
    ? acc
    : def(h)
    ? first([head(xs), ...tail(xs)], n - 1, [...acc, h])
    : acc;
const first2 = ([h, ...xs], n = 1) =>
  def(h) && n ? [h, ...first(xs, n - 1)] : [];
